...
public class MyHelpClaimItemListAdapter extends RecyclerView.Adapter<MyHelpClaimItemListAdapter.MyViewHolder>{
    ...
    public MyHelpClaimItemListAdapter(Context context, ArrayList<MyClaimItemListResBean.DataItem> ItemList, RejectClickListener listener) {
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        ...
        Picasso.get().load(ApiConstants.BASE_IMAGE_URL+ItemList.get(position).getImage()).placeholder(context.getResources().getDrawable(R.drawable.ic_ask_help)).into(holder.imgBook);
        ...
        if (ItemList.get(position).getClaimBy().equalsIgnoreCase(((MainActivity)context).profileData.getUser_id())){
            ...
            if (ItemList.get(position).getIsRequestRecieved().equalsIgnoreCase("1")){
        ...
        holder.txtReceived.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (ItemList.get(position).getIsRequestRecieved().equalsIgnoreCase("0")) {
